...
# nv-community/PhysicalAI-Robotics-mindmap-Franka-Mug-in-Drawer
# （Franka 单臂，Isaac Lab 遥操作 + Mimic 扩增，mindmap 格式含逐相机深度 PNG + 内参）
# 整仓 ~36.8GB：10 个 demo tar（每个 ~3-4GB）+ 250 条演示的 HDF5，到 data/ 下。
#
# 说明：
...
#
# 用法：
#   bash scripts/download_mindmap_franka_mug_in_drawer.sh
# 断点续传：中断后重新运行同一命令即可。公开仓库，无需登录。

set -euo pipefail

REPO="nv-community/PhysicalAI-Robotics-mindmap-Franka-Mug-in-Drawer"
OUT="data/mindmap_franka_mug_in_drawer"
