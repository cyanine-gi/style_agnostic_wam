...
# 2026VLMTaskPlanningClutter/GraspClutter6D-YCB-MuJoCo
# （GraspClutter6D + YCB 共 200 个物体的 MuJoCo MJCF 版：每个物体含 .xml MJCF、
#  可视化 obj + 纹理、凸分解碰撞体，可经 MJCF->USD 导入 IsaacLab 复刻抓取场景）
# 到 data/ 下。共 ~6.7k 个小文件。
#
# 两个坑及对策（同 download_physicalai_singlearm.sh）：
#   1) 仓库文件数 >1000，HF 列表 API 需翻页；hf-mirror 不改写分页 Link 头里的
#      官网绝对地址，裸用会直连 huggingface.co（国内不可达）。
#      -> 这里自己翻页，并把下一页地址重写回镜像。
#   2) hf-mirror 对匿名 API 有限流（HTTP 429），翻页容易触发。
#      -> 列表结果缓存到 .file_list.json，每页落盘；429 时退避等待重试，
#         中断后重跑从上次页码继续；列表完成后重跑直接用缓存，不再请求列表 API。
#
# 用法：
#   bash scripts/download_graspclutter6d_ycb_mujoco.sh
# 断点续传：中断后重新运行同一命令即可（列表和文件下载都可续）。公开仓库，无需登录。

set -euo pipefail

REPO="2026VLMTaskPlanningClutter/GraspClutter6D-YCB-MuJoCo"
OUT="data/graspclutter6d_ycb_mujoco"
